...
USAGE
    ...
    python extract_embeddings.py --split eval --batch 4 \
...
def transcode_g711(src: str, dst: str) -> None:
    ...
    The final file is written through <dst>.part and renamed only once ffmpeg
    has succeeded. Without that, a failed stage 2 or a Ctrl-C mid-write leaves a
    truncated .flac that the resume check below would count as finished forever.
    """
    ...
    try:
        ...
        subprocess.run(
            ["ffmpeg", "-y", "-loglevel", "error", "-i", src,
             "-ar", "8000", "-ac", "1", "-c:a", "pcm_mulaw", tmp_wav],
            check=True, capture_output=True)
        # stage 2: decode + resample back to 16 kHz, store as FLAC
        subprocess.run(
            # -f flac is required: the output name ends in .part, so ffmpeg
            # cannot infer the muxer from the extension
            ["ffmpeg", "-y", "-loglevel", "error", "-i", tmp_wav,
             "-ar", "16000", "-ac", "1", "-c:a", "flac", "-f", "flac", part],
            check=True, capture_output=True)
        os.replace(part, dst)
    finally:
        for leftover in (tmp_wav, part):
            try:
                os.unlink(leftover)
            except OSError:
                pass
...
def main() -> int:
    ...
    lines = [ln for ln in proto.read_text(encoding="utf-8").splitlines()
             if ln.strip()]

    def _by_filename(ln):
        parts = ln.split()
        return parts[1] if len(parts) >= 2 else ln

    lines.sort(key=_by_filename)
    if args.limit:
    ...
    for ln in tqdm(lines, desc=f"{args.codec}:{args.split}", unit="file"):
        ...
        try:
            ...
            print(f"\n  ! ffmpeg failed on {name}: {err}")
            failed += 1
        except Exception as e:          # one bad file is logged, never fatal
    ...
    print(f"next: python extract_embeddings.py --split {args.split} "
